...
import { getMyQuotaUsage, type QuotaType, type QuotaUsageItem } from "@/lib/api/quotas";
...
function bannerMessage(items: QuotaUsageItem[]): { severity: Severity; text: string } | null {
  const overHard = items.filter((i) => i.over_hard_limit);
  const overSoft = items.filter((i) => i.over_soft_limit && !i.over_hard_limit);
  const near = items.filter((i) => i.near_limit && !i.over_soft_limit && !i.over_hard_limit);

  if (overHard.length > 0) {
    const names = overHard.map((i) => QUOTA_TYPE_LABELS[i.quota_type as QuotaType] ?? i.quota_type).join(", ");
    return { severity: "error", text: `Hard limit reached for ${names}. Further actions are blocked until the limit resets.` };
  }
  if (overSoft.length > 0) {
    const names = overSoft.map((i) => QUOTA_TYPE_LABELS[i.quota_type as QuotaType] ?? i.quota_type).join(", ");
    return { severity: "warning", text: `Soft limit exceeded for ${names}. Contact your admin to increase limits.` };
  }
  if (near.length > 0) {
    const names = near.map((i) => QUOTA_TYPE_LABELS[i.quota_type as QuotaType] ?? i.quota_type).join(", ");
    return { severity: "warning", text: `Approaching quota limit for ${names}.` };
  }
  return null;
}
 ...
 */
export function QuotaWarningBanner({ filterTypes, className }: QuotaWarningBannerProps) {
  ...
  const items = filterTypes
    ? usageQuery.data.quota_usage.filter((i) => filterTypes.includes(i.quota_type as QuotaType))
