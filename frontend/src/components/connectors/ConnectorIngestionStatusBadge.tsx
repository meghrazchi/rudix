...
const STATUS_META: Record<ConnectorFileStatus, StatusMeta> = {
  ...
  pending_scan: {
    label: "Pending scan",
    description: "File has been received and is waiting for the security scan to run.",
    colorClass: "bg-yellow-100 text-yellow-800",
  },
  infected: {
    label: "Infected",
    description: "ClamAV detected malware in this file. It will not be indexed.",
    colorClass: "bg-red-100 text-red-800",
  },
  extraction_failed: {
    label: "Extraction failed",
    description: "Text could not be extracted from this file. Check the document detail for diagnostics.",
    colorClass: "bg-red-100 text-red-700",
  },
  ocr_applied: {
    label: "OCR applied",
    description: "The file was processed with OCR because it contained scanned pages.",
    colorClass: "bg-teal-100 text-teal-800",
  },
  skipped: {
    label: "Skipped",
    description: "This file was not re-indexed because an identical copy already exists in the workspace.",
    colorClass: "bg-gray-100 text-gray-600",
  },
  unsupported: {
    label: "Unsupported",
    description: "This file format is not supported for indexing and was not ingested.",
    colorClass: "bg-gray-100 text-gray-500",
  },
  // --- Shared pipeline statuses ---
  uploaded: {
    label: "Uploaded",
    description: "File has been uploaded and is waiting for the processing pipeline.",
  ...
  indexed: {
    label: "Indexed",
    description: "File has been fully indexed and is available for search and citations.",
    colorClass: "bg-green-100 text-green-800",
  },
  failed: {
    label: "Failed",
    description: "An error occurred during processing. The document may be retried.",
    colorClass: "bg-red-100 text-red-800",
  },
  quarantined: {
    label: "Quarantined",
...
export function ConnectorIngestionStatusBadge({
  ...
  const meta =
    STATUS_META[status as ConnectorFileStatus] ?? FALLBACK_META;

  return (
    <span
      title={showTooltip ? meta.description : undefined}
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium cursor-default ${meta.colorClass} ${className}`}
    >
      {meta.label}
    </span>
  );
...
export function ConnectorIngestionStatusRow({
  ...
  const isTerminalError = ["infected", "extraction_failed", "failed", "blocked"].includes(status);

  return (
    <div className={`flex flex-col gap-1 ${className}`}>
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-gray-500 w-20 shrink-0">Status</span>
        <ConnectorIngestionStatusBadge status={status} />
      </div>
      {isTerminalError && errorMessage && (
        <div className="flex items-start gap-2">
          <span className="text-xs font-medium text-gray-500 w-20 shrink-0">Error</span>
