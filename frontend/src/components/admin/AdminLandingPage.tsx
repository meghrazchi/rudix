  ...
  return [
    {
      ...
      description: "Inspect token usage, estimated cost, and performance trends.",
      href: "/admin/usage",
      available: true,
    ...
    },
    {
      id: "system-health",
      title: "System health",
      description: "Check API health and readiness from the deployed environment.",
  ...
  return (
    <section className="space-y-6 px-4 py-5 lg:px-8 lg:py-8">
      <header className="rounded-2xl border border-[#d7d4e8] bg-white p-5 shadow-sm">
        ...
        <h1 className="mb-2 text-2xl font-extrabold text-[#2a2640] lg:text-3xl">Admin landing</h1>
        <p className="max-w-3xl text-sm text-[#68647b]">
          Open usage, audit, health, and monitoring surfaces with role-aware access controls.
        </p>
      </header>
          ...
          return (
            <article key={card.id} className="rounded-2xl border border-[#d7d4e8] bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <h2 className="text-lg font-bold text-[#2a2640]">{card.title}</h2>
                <span
                  className={`rounded-full px-2 py-1 text-[11px] font-semibold uppercase tracking-wide ${
                    card.available ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                  }`}
                >
