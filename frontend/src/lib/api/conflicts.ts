...
function normalizeConflictEntry(value: unknown): ConflictEntry {
  ...
  return {
    ...
    subject_value:
      typeof raw.subject_value === "string" ? raw.subject_value : "",
    ...
    conflict_type:
      typeof raw.conflict_type === "string" ? raw.conflict_type : "",
    ...
    conflict_summary:
      typeof raw.conflict_summary === "string" ? raw.conflict_summary : null,
    ...
    remediation: Array.isArray(raw.remediation)
      ? (raw.remediation as unknown[]).filter(
...
export async function listConflicts(params?: {
  ...
  return {
    items: Array.isArray(raw.items)
      ? raw.items.map(normalizeConflictEntry)
      : [],
...
export async function scanForConflicts(): Promise<ScanResult> {
  ...
  return {
    conflicts_detected:
      typeof raw.conflicts_detected === "number" ? raw.conflicts_detected : 0,
    conflicts_created:
      typeof raw.conflicts_created === "number" ? raw.conflicts_created : 0,
    scan_duration_ms:
      typeof raw.scan_duration_ms === "number" ? raw.scan_duration_ms : 0,
    scanned_grants:
      typeof raw.scanned_grants === "number" ? raw.scanned_grants : 0,
    scanned_denies:
      typeof raw.scanned_denies === "number" ? raw.scanned_denies : 0,
    scanned_acl_mappings:
      typeof raw.scanned_acl_mappings === "number"
        ? raw.scanned_acl_mappings
        : 0,
  };
}
...
export async function explainDecision(params: {
  ...
  const payload = await apiRequest<unknown>(`${BASE}/explain-decision?${qs}`, {
    method: "GET",
    retry: false,
  });
  const raw =
  ...
  return {
    ...
    subject_user_id:
      typeof raw.subject_user_id === "string" ? raw.subject_user_id : "",
    resource_type:
      typeof raw.resource_type === "string" ? raw.resource_type : "",
    resource_id: typeof raw.resource_id === "string" ? raw.resource_id : null,
    action: typeof raw.action === "string" ? raw.action : "",
