...
function normalizeRoleMatrixEntry(value: unknown): RoleMatrixEntry {
  ...
  return {
    ...
    overridden_permissions: Array.isArray(raw.overridden_permissions)
      ? (raw.overridden_permissions as unknown[]).filter((p): p is string => typeof p === "string")
      : [],
  };
}
...
function normalizeResourceAccessEntry(value: unknown): ResourceAccessEntry {
  ...
  return {
    ...
    principal_type: typeof raw.principal_type === "string" ? raw.principal_type : "",
    principal_value: typeof raw.principal_value === "string" ? raw.principal_value : "",
    ...
    created_by_user_id: typeof raw.created_by_user_id === "string" ? raw.created_by_user_id : null,
...
export async function getRoleMatrix(): Promise<RoleMatrixResponse> {
  ...
  return {
    roles: Array.isArray(raw.roles) ? raw.roles.map(normalizeRoleMatrixEntry) : [],
    all_permissions: Array.isArray(raw.all_permissions)
      ? (raw.all_permissions as unknown[]).filter((p): p is string => typeof p === "string")
      : [],
  };
}
...
export async function listResourceGrants(params?: {
  ...
  const raw = payload && typeof payload === "object" ? (payload as Record<string, unknown>) : {};
  return {
    items: Array.isArray(raw.items) ? raw.items.map(normalizeResourceAccessEntry) : [],
...
export async function revokeResourceGrant(grantId: string): Promise<void> {
  await apiRequest<unknown>(`${BASE}/resource-grants/${encodeURIComponent(grantId)}`, {
    method: "DELETE",
    retry: false,
  });
}
...
export async function revokeResourceDeny(denyId: string): Promise<void> {
  await apiRequest<unknown>(`${BASE}/resource-denies/${encodeURIComponent(denyId)}`, {
    method: "DELETE",
    retry: false,
  });
}
