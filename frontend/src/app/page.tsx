...
const capabilities: CapabilityCard[] = [
  {
    ...
    description: "Visual observability for your entire RAG lifecycle, from ingestion to generation.",
  },
  {
    icon: "ingestion",
    title: "Secure Ingestion",
    description: "Connect local files and cloud sources with policy-aware processing and auditability.",
  },
  {
    icon: "evaluation",
    title: "Automated Evaluation",
    description: "Benchmark retrieval and answer quality against evaluation sets and metrics.",
  },
];
...
const securityCards: SecurityCard[] = [
  ...
  {
    title: "Private VPC",
    description: "Deployment options for fully isolated private-cloud infrastructure.",
  },
  {
...
const trustLogos = ["Fortune 500", "Tech Giant", "Cyber Sec", "Global Fin"] as const;
...
function resolveLandingLinks(): LandingLinks {
  const docsUrl =
    ...
    "/documents";
  const demoUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_DEMO_URL) ?? "/login";
  const trialUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_TRIAL_URL) ?? "/signup";
  const productUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_PRODUCT_URL) ?? "/dashboard";
  const solutionsUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_SOLUTIONS_URL) ?? "/documents";
  const pricingUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_PRICING_URL) ?? "/settings";
  const statusUrl = trimToNull(process.env.NEXT_PUBLIC_LANDING_STATUS_URL) ?? "/admin/system-health";
  const contactUrl =
...
function ActionLink({
  ...
  return (
    <Link href={href} className={`${className} ${FOCUS_RING_CLASS}`} aria-label={ariaLabel}>
      {children}
    </Link>
  );
}
...
function StatusPill({ label }: { label: string }) {
  return (
    <span className="rounded-full border border-[#e2def7] bg-[#f2f0ff] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.12em] text-[#5a53a4]">
      {label}
    </span>
  );
}
    ...
    <main className="min-h-screen bg-[#f2f3f6] text-[#13141a]">
      <header className="border-b border-[#dbdde4] bg-[#f2f3f6]/95">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-4 lg:px-8">
          <div className="flex items-center gap-8">
            <Link href="/" className={`flex items-center gap-2 ${FOCUS_RING_CLASS}`} aria-label="Rudix home">
              <Image src={LOGO_MARK_SRC} alt="Rudix logo" width={24} height={24} className="h-6 w-6" />
      ...
      >
        <div>
          ...
          <h1 id="landing-hero-title" className="mt-6 text-4xl font-black leading-tight text-[#101218] lg:text-6xl">
            Scale Precision AI
            <br />
            with <span className="text-[#3a35e8]">Confidence.</span>
          </h1>
          <p className="mt-5 max-w-xl text-sm leading-7 text-[#505465] lg:text-base">
            Deploy secure, production-ready Enterprise-grade RAG infrastructure. Orchestrate complex data pipelines
            and transform unstructured knowledge into high-precision intelligence.
          </p>
        ...
        >
          ...
          <figcaption className="sr-only">
            Pipeline Explorer sample includes ingestion, chunking, storage, retrieval, reranking, and answer
            generation with trace details.
          </figcaption>
        </figure>
      </section>
      ...
      <section className="border-y border-[#dde0e8] bg-[#eff1f5] py-8">
        <div className="mx-auto w-full max-w-7xl px-4 text-center lg:px-8">
          <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#666a7d]">Powering Enterprise Intelligence</p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-8">
            {trustLogos.map((logo) => (
              <p key={logo} className="text-xs font-semibold uppercase tracking-[0.08em] text-[#7a7f91]">
                {logo}
              </p>
            ))}
          </div>
      ...
      <section id="capabilities" aria-labelledby="capabilities-title" className="mx-auto w-full max-w-7xl px-4 py-14 lg:px-8 lg:py-20">
        <div className="text-center">
          <h2 id="capabilities-title" className="text-3xl font-black text-[#12141b] lg:text-5xl">
            Native RAG Capabilities
          </h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-[#595d6e]">
            The infrastructure layer designed to handle the complexity of enterprise data and LLM orchestration.
          </p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {capabilities.map((item) => (
            <article key={item.title} className="rounded-2xl border border-[#d8dce7] bg-white p-5 shadow-sm">
              ...
              <h3 className="text-2xl font-semibold text-[#171a24]">{item.title}</h3>
              <p className="mt-3 text-sm leading-7 text-[#5a6071]">{item.description}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section aria-labelledby="engineering-title" className="bg-[#1e242f]">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-14 lg:grid-cols-2 lg:items-center lg:gap-12 lg:px-8 lg:py-20">
          <figure role="img" aria-label="Terminal-like code sample showing Rudix client retrieval flow" className="rounded-xl border border-[#303847] bg-[#111722] p-4 shadow-[0_20px_40px_rgba(0,0,0,0.35)]">
...
print(query.answer_confidence)`}
          ...
          <div>
            <h2 id="engineering-title" className="text-4xl font-black text-white lg:text-5xl">
              Built for Engineering Excellence
            </h2>
            <p className="mt-4 text-sm leading-7 text-[#c7cede] lg:text-base">
              Deploy enterprise-grade RAG infrastructure with APIs and clients that integrate into your existing
              product and DevOps workflows.
            </p>
            <ul className="mt-6 space-y-4 text-sm text-[#dce3f2]">
              <li className="rounded-lg border border-[#384154] bg-[#252d3b] px-4 py-3">
                GitLab-integrated dashboards and version-controlled prompt pipelines.
              </li>
              <li className="rounded-lg border border-[#384154] bg-[#252d3b] px-4 py-3">
      ...
      <section aria-labelledby="security-title" className="mx-auto w-full max-w-7xl px-4 py-14 lg:px-8 lg:py-20">
        <h2 id="security-title" className="text-center text-3xl font-black text-[#12141b] lg:text-5xl">
          Security First Infrastructure
        </h2>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {securityCards.map((item) => (
            <article key={item.title} className="rounded-xl border border-[#d8dce7] bg-white px-4 py-5 text-center shadow-sm">
              <p className="text-sm font-bold text-[#30344a]">{item.title}</p>
              <p className="mt-2 text-xs leading-6 text-[#676c7f]">{item.description}</p>
            </article>
          ))}
        </div>
      </section>

      <section aria-labelledby="cta-title" className="bg-[linear-gradient(135deg,#2a2fe3_0%,#251bc0_52%,#3b1ed0_100%)]">
        <div className="mx-auto w-full max-w-7xl px-4 py-16 text-center lg:px-8 lg:py-24">
          <h2 id="cta-title" className="text-4xl font-black text-white lg:text-6xl">
            Deploy Production RAG Today
          </h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-[#d8dcff] lg:text-base">
            Join engineering teams using Rudix to build secure, observable, and high-confidence AI experiences.
          </p>
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 py-10 lg:grid-cols-[1.3fr_1fr_1fr_1fr] lg:px-8">
          <div>
            ...
            <p className="mt-3 max-w-xs text-xs leading-6 text-[#626778]">
              Enterprise-grade RAG infrastructure for secure, scalable knowledge retrieval and AI applications.
            </p>
            <p className="mt-4 text-xs text-[#7c8194]">© 2026 Rudix AI. All rights reserved.</p>
          </div>

          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-[#4f5467]">Product</p>
            <ul className="mt-3 space-y-2 text-sm text-[#4b4f60]">
              <li>
                <ActionLink href="/rag-pipeline" className="transition hover:text-[#25283a]">
                  Pipeline Explorer
                </ActionLink>
              </li>
              <li>
                <ActionLink href={links.readDocs} className="transition hover:text-[#25283a]">
                  API Reference
                </ActionLink>
              </li>
              <li>
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-[#4f5467]">Company</p>
            <ul className="mt-3 space-y-2 text-sm text-[#4b4f60]">
              <li>
                <ActionLink href={links.product} className="transition hover:text-[#25283a]">
                  About Us
                </ActionLink>
              </li>
              <li>
                <ActionLink href={links.solutions} className="transition hover:text-[#25283a]">
                  Careers
                </ActionLink>
              </li>
              <li>
                <ActionLink href="/forbidden" className="transition hover:text-[#25283a]">
                  Trust Center
                </ActionLink>
              </li>
            </ul>
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-[#4f5467]">Support</p>
            <ul className="mt-3 space-y-2 text-sm text-[#4b4f60]">
              <li>
                <ActionLink href={links.readDocs} className="transition hover:text-[#25283a]">
                  Documentation
              ...
              <li>
                <ActionLink href={links.footerStatus} className="transition hover:text-[#25283a]">
                  Status Page
                </ActionLink>
              </li>
              <li>
                <ActionLink href={links.footerContact} className="transition hover:text-[#25283a]">
                  Contact
                </ActionLink>
              </li>
            </ul>
