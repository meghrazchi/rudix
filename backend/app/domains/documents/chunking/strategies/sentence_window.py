...
# uppercase letter or digit.  Does not attempt abbreviation detection; the
# goal is "good enough" splitting without external NLP dependencies.
_SENTENCE_END = re.compile(r"(?<=[.!?…])\s+(?=[A-Z\d\"\'])")
_PARA_SPLIT = re.compile(r"\n{2,}")


def _split_sentences(page_number: int, text: str) -> list[tuple[int, str]]:
    """Yield (page_number, sentence) pairs without leaking raw text into logs."""
    ...
    for paragraph in _PARA_SPLIT.split(text):
        ...
        for part in _SENTENCE_END.split(para):
            s = part.strip()
            if s:
                sentences.append((page_number, s))
    return sentences


class SentenceWindowStrategy:
    """Groups sentences into token-budget windows with sentence-level overlap.

    Sentences are detected with a regex-based rule that does not require an
    external NLP library and does not crash on multilingual text (it simply
    keeps non-split text as single units).

    Overlap is computed by including enough trailing sentences from the
    previous window to fill *chunk_overlap_tokens* tokens; the overlap may
    slightly exceed the target since full sentences are kept intact.

    Sentences individually exceeding *chunk_size_tokens* are split token-wise,
    matching the behavior of paragraph_recursive.
    """
    ...
    async def chunk(
        ...
        *,
        ...
        for page in ordered_pages:
            for pnum, text in _split_sentences(page.page_number, page.text):
