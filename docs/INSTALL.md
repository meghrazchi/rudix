...
- MCP settings (optional, disabled by default):
  ...
  - `MCP_CAPABILITIES_OWNER` / `MCP_CAPABILITIES_ADMIN` / `MCP_CAPABILITIES_MEMBER` / `MCP_CAPABILITIES_VIEWER`
  - `MCP_RATE_LIMIT_ENABLED` / `MCP_RATE_LIMIT_WINDOW_SECONDS` / `MCP_RATE_LIMIT_REQUESTS`
...
# List chat sessions for active org/user scope
curl -sS "http://localhost:8000/api/v1/chat/sessions?limit=10&offset=0" \
  -H "Authorization: Bearer $TOKEN" \
  -H "X-Organization-ID: $ORG_ID" | jq

# Fetch chat session detail
